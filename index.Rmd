...
```{r Guns, echo= F, include=T, warning=F,  fig.align = "center", fig.cap = "The Search Trend of Guns Across States"}
for(i in 1:6){
...
#### Discussion: as the plots show, the search terms, Nintendo Switch, Sourdough, and Gun, become increasing popular since March 20, which is the period that first wave of states started lockdowned. These search trends reflect that after lockdown, people have to stay at home. Their attention shifts to home entertainment and safety. And partisanship does not affect how people live their life.  
...
#### Discussion: in contrast, some search terms, such as Cheap Tickets, Basketball, are on the wane. After lockdown, international or domestic travel is banned or restricted, which maybe the reason why people search cheap tickets less than before. NBA announced the decision to suspend season on March 12. Therefore, the search trend of basketball is down since then. Again, we do not observe any different search trend patterns across states. 
...
#### Discussion: we also found that, some terms are very sensitive to lockdown. For example, Business search spiked around the start date of lockdown in each state (except Texas). 
...
#### Discussion: in contrast, some terms, their search trends are not affected by lockdown. For instance, Face Mask search did not increase until CDC suggest people to wear for protection at the begining of April. 
...
#### Discussion: none of the above terms show different patterns of search trend across states. So we tried some partisan terms, like Protest and First Amendment, but it is still hard to find the association between partsianship and search trend. Although there maybe individual level partisan differences, it is really hard to detect any particular search trend pattern at state level based on partisan support.
